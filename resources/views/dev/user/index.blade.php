          ...
          <div class="card-header">
            ...
            <div class="card-header-action">
              <button type="button" class="btn btn-warning" data-toggle="modal" data-target="#modalImport">
                <i class="fas fa-upload"></i> Import Peminjam
              </button>
              <a href="{{ url('dev/user/export') }}" class="btn btn-success">
                <i class="fas fa-download"></i> Download Format Excel
              </a>
              |
              <a href="{{ url('dev/user/trash') }}" class="btn btn-success btn-sm">
            ...
            <div class="p-4">
              <form action="{{ url('dev/user') }}" method="GET" id="get-filter">
                <div class="float-left mb-3 mr-3" style="width: 140px">
                  <select class="form-control selectric" name="role" onchange="event.preventDefault();
                  document.getElementById('get-filter').submit();">
                    <option value="" {{ Request::get('role')=='' ? 'selected' : null }}>Semua</option>
                    <option value="admin" {{ Request::get('role')=='admin' ? 'selected' : null }}>Admin</option>
