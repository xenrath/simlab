            ...
            <div class="card-header">
              ...
              <div class="card-header-action dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-sm dropdown-menu-right">
                  ...
                  <li>
                    <a href="{{ url('dev/user/export') }}" class="dropdown-item">Download Format Excel</a>
                  </li>
                  <li>
                    <a href="#" class="dropdown-item" data-toggle="modal" data-target="#modalImport">Import User</a>
                  </li>
                  <li>
                ...
                <form action="{{ url('dev/user') }}" method="GET" id="get-filter">
                  <div class="float-left mb-3 mr-3" style="width: 160px">
                    <select class="form-control selectric" name="filter"
                      ...
                      <option value="role" {{ Request::get('filter') == 'role' ? 'selected' : null }}>Role User</option>
                      <option value="updated_at" {{ Request::get('filter') == 'updated_at' ? 'selected' : null }}>
                        Terakhir
                        Diubah</option>
                    </select>
                  </div>
                    ...
                    @forelse($users as $user)
                      <tr>
                        ...
                        <td class="text-center">
                          <form action="{{ url('dev/user/' . $user->id) }}" method="POST" id="del-{{ $user->id }}">
                            @csrf
                            @method('delete')
                            <a href="{{ url('dev/user/' . $user->id) }}" class="btn btn-info">
                              <i class="fas fa-eye"></i>
                            </a>
                            <a href="{{ url('dev/user/' . $user->id . '/edit') }}" class="btn btn-warning">
            ...
            <div class="form-group">
              ...
              <select class="form-control selectric" name="subprodi_id" id="subprodi_id">
                ...
                @foreach ($subprodis as $subprodi)
                  <option value="{{ $subprodi->id }}">{{ $subprodi->jenjang }} {{ $subprodi->nama }}</option>
                @endforeach
              </select>
            </div>
          </div>
