...
@section('content')
    <section class="section">
        ...
        <div class="section-body">
                ...
                <div class="card-header">
                    ...
                    <div class="card-header-action">
                        @php
                            ...
                            $tanggal_awal = date('Y-m-d', strtotime($pinjam->tanggal_awal));
                            $tanggal_akhir = date('Y-m-d', strtotime($pinjam->tanggal_akhir));
                        @endphp
            ...
            <form action="{{ url('peminjam/normal/peminjaman/' . $pinjam->id) }}" method="POST" autocomplete="off">
                @csrf
                @method('put')
                @if ($error_barang)
                        ...
                        <select class="custom-select custom-select-sm mr-2 w-100" id="keyword_ruang_id"
                            ...
                            @foreach ($ruangs as $ruang)
                                <option value="{{ $ruang->id }}"
                                    {{ $pinjam->ruang_id == $ruang->id ? 'selected' : null }}>
                                    {{ $ruang->nama }}</option>
                            @endforeach
                        </select>
                    </div>
    ...
    <script type="text/javascript">
        ...
        function modal_items(data, is_selected) {
            ...
            card_items +=
                ...
                '" onclick="add_item(' + data.id + ', ' + detail_pinjam_id + ')" ' + checked + ' >';
